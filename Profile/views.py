...
from . forms import UserRegisterForm, UserLoginForm, PaymentForm, TenantProfileForm, UserHostelPlanForm, HostelPaymentplanForm
...
# 
from . models import PaymentRequestHeaders, TenantProfile, UserHostelPlan, HostelPaymentplan
...
def all_user_transaction(request):
    context = {}
    if request.user.is_authenticated:
        previous_transaction = PaymentRequestHeaders.objects.filter(user=request.user.id)
...
def payments(request):
    ...
    if request.user.is_authenticated:
        ...
        previous_transaction = PaymentRequestHeaders.objects.filter(user=request.user.id)
        hostel_plan_check_qs = UserHostelPlan.objects.filter(tenant=request.user.id) #tenant->user request.user.landlordprofile.id
        hostel_payment_plan_check_qs = HostelPaymentplan.objects.filter(tenant=request.user.id)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"{plan_form.cleaned_data.get('plan')} Selected Successfully") 
            obj = plan_form.save(commit = False)
            obj.tenant_id = request.user.id
            obj.save()
            return redirect('/user/payments/')
        elif hostel_payment_plan_form.is_valid():
            messages.success(request, f"{request.POST['amountpaid']} Selected Successfully") 
